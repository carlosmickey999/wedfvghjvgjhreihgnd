...
def start(update, context):
    ...
    buttons.buildbutton("Repo(not Available)", "https://www.google.com")
    buttons.buildbutton("Owner", "https://t.me/GParle")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
This bot can mirror all your links to Google Drive!
Can easily do : Leeching torrent mega GdToT Cloning Unzip and much more 
'''
        sendMarkup(start_string, context.bot, update, reply_markup)
    else:
        sendMarkup(
            'Oops! not a Authorized user.\nGet permission from the Owner to use.',
            context.bot,
            update,
            reply_markup,
        )
